            ...
            <tr v-for="loan in loans" :key="loan.loanID" class="hover:bg-gray-50 transition-colors duration-150">
              ...
              <td class="px-6 py-4 whitespace-nowrap">
                <div class="flex space-x-2">
                  ...
                  <!-- Show status-specific action buttons -->
                  <button
                    v-if="loan.status === 'approved' || loan.status === 'active'"
                    @click="updateLoanStatus(loan.loanID, 'completed')"
                    :disabled="updatingStatus === loan.loanID"
                    class="inline-flex items-center px-3 py-2 border border-transparent text-xs font-medium rounded-md text-white bg-blue-600 hover:bg-blue-700 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 disabled:opacity-50 disabled:cursor-not-allowed transition-colors duration-150"
                  >
                    <CheckIcon class="h-3 w-3 mr-1" />
                    {{ updatingStatus === loan.loanID ? 'Updating...' : 'Mark Complete' }}
                  </button>
    ...
    <div v-if="selectedLoan" class="fixed inset-0 bg-gray-600 bg-opacity-50 overflow-y-auto h-full w-full z-50">
      <div class="relative top-10 mx-auto p-5 border w-11/12 md:w-3/4 lg:w-1/2 shadow-2xl rounded-lg bg-white">
        <div class="mt-3">
          ...
          <!-- Status-specific action buttons in modal -->
          ...
          <div v-else-if="selectedLoan.status === 'approved' || selectedLoan.status === 'active'" class="mt-8 pt-6 border-t border-gray-200">
            <div class="flex space-x-4">
              <button
                @click="updateLoanStatus(selectedLoan.loanID, 'completed')"
                :disabled="updatingStatus === selectedLoan.loanID"
                class="flex-1 bg-blue-600 text-white px-6 py-3 rounded-lg hover:bg-blue-700 disabled:opacity-50 focus:outline-none focus:ring-2 focus:ring-offset-2 focus:ring-blue-500 transition-colors duration-150 flex items-center justify-center"
              >
                <CheckIcon class="h-5 w-5 mr-2" />
                {{ updatingStatus === selectedLoan.loanID ? 'Updating...' : 'Mark as Complete' }}
              </button>
            </div>
          </div>
          
          <div v-else-if="selectedLoan.status === 'completed' || selectedLoan.status === 'rejected'" class="mt-8 pt-6 border-t border-gray-200">
            <div class="bg-gray-50 p-4 rounded-lg">
              <p class="text-sm text-gray-600 text-center">
                <span class="font-medium">{{ selectedLoan.status.charAt(0).toUpperCase() + selectedLoan.status.slice(1) }}</span> loans cannot be modified.
              </p>
            </div>
          </div>
        </div>
