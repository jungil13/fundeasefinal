...
async listLoans(req, res) {
  ...
  async updateLoanStatus(req, res) {
    try {
      ...
      // Get current loan status to check if it can be updated
      const currentLoan = await LoanModel.getLoanById(loanID);
      if (!currentLoan) {
        return res.status(404).json({ success: false, message: "Loan not found" });
      }

      // Restrict status changes - only pending loans can be approved/rejected
      if (currentLoan.status !== 'pending' && (status === 'approved' || status === 'rejected')) {
        return res.status(400).json({ 
          success: false, 
          message: `Cannot change status from ${currentLoan.status} to ${status}. Only pending loans can be approved or rejected.` 
        });
      }

      // Restrict status changes for completed/rejected loans
      if (currentLoan.status === 'completed' || currentLoan.status === 'rejected') {
        return res.status(400).json({ 
          success: false, 
          message: `Cannot change status of ${currentLoan.status} loans.` 
        });
      }
