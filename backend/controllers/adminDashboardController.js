...
export const getAdminDashboard = async (req, res) => {
  try {
    ...
    // Get fund balance (total confirmed contributions - total approved withdrawals + confirmed loan repayments)
    ...
    const fundBalance = parseFloat(contributionsResult[0]?.totalContributions || 0) - 
    ...
    const [monthlyLoanRepaymentsResult] = await pool.query(`
    ...
    const dashboardData = {
      ...
      data: {
        stats: {
          ...
          totalContributions: {
            value: `₱ ${parseFloat(totalContributions).toLocaleString()}`,
          ...
          loanRepayments: {
            value: `₱ ${parseFloat(monthlyLoanRepaymentsResult[0]?.totalRepayments || 0).toLocaleString()}`,
