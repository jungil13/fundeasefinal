...
export const getMemberDashboard = async (req, res) => {
  try {
    ...
    // Get member's loan stats - FIXED CALCULATION
    const [loanStats] = await pool.query(
      `SELECT 
        COUNT(*) as totalLoans,
        SUM(CASE WHEN status IN ('approved', 'disbursed', 'active') THEN amount ELSE 0 END) as activeLoanAmount,
        SUM(CASE WHEN status IN ('approved', 'disbursed', 'active') THEN remainingBalance ELSE 0 END) as outstandingBalance,
    ...
    // Get member's confirmed loan repayments
    const [loanRepaymentsResult] = await pool.query(
      `SELECT 
        COALESCE(SUM(lr.amount), 0) as totalLoanRepayments,
        COALESCE(SUM(CASE WHEN MONTH(lr.paymentDate) = MONTH(CURRENT_DATE()) AND YEAR(lr.paymentDate) = YEAR(CURRENT_DATE()) THEN lr.amount ELSE 0 END), 0) as monthlyLoanRepayments
    ...
    // Get member's fund balance (member's contributions - member's withdrawals + member's loan repayments)
    ...
    const [memberWithdrawals] = await pool.query(
    ...
    const memberFundBalance = parseFloat(memberContributions[0]?.totalContributions || 0) - 
                             parseFloat(memberWithdrawals[0]?.totalWithdrawals || 0) +
    ...
    const dashboardData = {
      ...
      data: {
        stats: {
          fundBalance: {
            value: `₱ ${memberFundBalance.toLocaleString()}`,
          ...
          loanRepayment: {
            value: `₱ ${parseFloat(loanRepaymentsResult[0]?.monthlyLoanRepayments || 0).toLocaleString()}`,
